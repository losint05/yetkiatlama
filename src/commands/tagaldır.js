...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    msg.react("843247993364545596"); //onay
    msg.react("843248099723182112"); //carpi

    msg.awaitReactions((reaction, user) => ["843247993364545596", "843248099723182112"].includes(reaction.emoji.name) && user.id === member.user.id, {
      max: 1,
      time: 30000,
      errors: ['time']
    }).then(async collected => {
      const reaction = collected.first();
      if (reaction.emoji.name === '843247993364545596') {
