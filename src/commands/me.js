...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    **───────────────**
    **➥ Ses Bilgileri:**
  ${conf.nokta} Toplam: \`${moment.duration(voiceData ? voiceData.topStat : 0).format("H [saat], m [dakika] s [saniye]")}\`
  ${conf.nokta} Public Odalar: \`${await category(conf.publicParents)}\`
  ${conf.nokta} Kayıt Odaları: \`${await category(conf.registerParents)}\`
  ${conf.nokta} Sorun Çözme & Terapi: \`${await category(conf.solvingParents)}\`
  ${conf.nokta} Private Odalar: \`${await category(conf.privateParents)}\`
  ${conf.nokta} Game Odalar: \`${await category(conf.aloneParents)}\`
  ${conf.nokta} Oyun & Eğlence Odaları: \`${await category(conf.funParents)}\`
  ${conf.nokta} Diğer: \`${await category(filteredParents.map(x => x.id))}\`
    **───────────────**
    ...
    **───────────────**
    ...
    ${conf.nokta} Haftalık Ses: \`${voiceWeekly}\`
    ${conf.nokta} Günlük Ses: \`${voiceDaily}\`
    `, true);
